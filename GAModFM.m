function subExperiments = GAModFM(targetWav,fs,nbCarriers,nbHarm,Bw,Ns,idExp,idSubExp)
%GAModFM Modified FM model optimization using Genetic Algorithm
%   GAClassicFM generates a waveform using the general equation of the FM
%   synthesis method.
%
%subExperiments = GAModFM(targetWav,fs,nbCarriers,nbHarm,Bw,Ns,idExp,idSubExp)
...
%  also the result and target signals and harmonics list. 
    subExperiments = {};
            ...
            [subExperiments.resultSynth,subExperiments.fval,subExperiments.exitflag,subExperiments.output] = ga(@ModFMsimplefitness5_par,2*nbCarriers,[],[],[],[],LB,UB,[],intConst,options)

            function [result,output,W,harmCar] = ModFMsimplefitness5_par( params )
                %ModFMsimplefitness Compute the fitness of a individual
                ...
                for k=1:nbCarriers
                    T= [T ; ModFMSynth(params(2*k-1)*fm,fm,1,params(2*k),nbSamples,fs)];
            ...
            [subExperiments.fitResult,resultSound,subExperiments.resultEnv,harmCar] = ModFMsimplefitness5_par( subExperiments.resultSynth );
